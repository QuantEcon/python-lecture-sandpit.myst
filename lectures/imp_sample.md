...
Our goal is to approximate the mathemataical expectsion $E \left[ L\left(\omega^t\right) \right]$ well.
...
Beta distributions assumed above, $L\left(\omega^t\right)$ has  a very skewed distribution with a very long tail as $t \rightarrow \infty$.
...
In this lecture we explore how a standard Monte Carlo method fails and how **importance sampling** provides a more computationally efficient way to approximate
the mean of the cumulative likelihood ratio.
...
A Monte Carlo approximation of $\hat{E} \left[L\left(\omega^t\right)\right] = \hat{E} \left[\prod_{i=1}^t \ell \left(\omega_i\right)\right]$  would repeatedly draw $\omega$ from $g$ and calculate the likelihood ratio $ \ell(\omega) = \frac{f(\omega)}{g(\omega)}$ for each draw. 

Because $g(\omega) \rightarrow 0$ as $\omega \rightarrow 0$, our simulation  undersamples a part of the sample space $[0,1]$ that it is important to visit often in order to do a good job of approximating the mathematical expectation of the likelihood ratio $\ell(\omega)$.  
...
We now study how to use importance sampling to approximate ${E} \left[L(\omega^t)\right] = \left[\prod_{i=1}^T \ell \left(\omega_i\right)\right]$. 
...
Our simulations suggest that indeed $h_2$ is a quite  good importance sampling distrbution for our problem.
...
However,  $h_3$ is evidently a poor importance sampling distribution forpir problem, with a mean estimate far away from $1$ for $T = 20$.
