...
```{admonition} Due Date
This is **due on Friday, April 30th**
```
...
that satisfies {eq}`equation-tss-techr-opt-tax` at all $ t, s^t $.
...
The household chooses $ \{c_t(s^t), \ell_t(s^t)\}_{t=0}^\infty $ to maximize {eq}`equation-ts-prefr-opt-tax` subject to {eq}`equation-ts-bcr` and {eq}`equation-feas1-opt-tax` for all $ t, s^t $.
...
A **Hamilton-Ramsey problem** or **optimal taxation, debt scheduling problem** is to choose a competitive equilibrium with distorting taxes that maximizes {eq}`equation-ts-prefr-opt-tax`.
...
1. Obtain  first-order conditions of the household’s problem and solve them for $ \{q^0_t(s^t), \tau_t(s^t)\}_{t=0}^\infty $
   as functions of the allocation $ \{c_t(s^t), n_t(s^t)\}_{t=0}^\infty $.
...
3. Find the allocation that maximizes the utility of the representative household {eq}`equation-ts-prefr-opt-tax` subject
   to the feasibility constraints {eq}`equation-feas1-opt-tax`  and {eq}`equation-tss-techr-opt-tax`  and the implementability
   condition derived in step 2.
   - This optimal allocation is called the **Hamilton-Ramsey allocation**.
4. Use the allocation together with the formulas from step 1 to find  taxes and prices.s
...
Using the first-order conditions {eq}`equation-lsa-taxr` and {eq}`equation-ls101` to eliminate taxes and prices from {eq}`equation-ts-bcpv2`, we derive the *implementability condition*
...
subject to {eq}`equation-tss-cham1`.
...
$$
b_t(s^t) = \tau_t(s^t) n_t(s^t) - g_t(s^t)
$$ (equation-exercise2)
...
**Exercise 3:** Please interpret inequality {eq}`equation-tss-cham1` and rule {eq}`equation-exercise2` as asserting that at each date and history, the value of government debt equals the  present value of government surpluses, so that ``large government debts **now** signal large government **surpluses** later''.

**Exercise 4:** Please interpret the government debt plan {eq}`equation-ts-prefr-opt-tax` as an insurance contract in which the government has purchased  insurance against **fiscal risk**.
