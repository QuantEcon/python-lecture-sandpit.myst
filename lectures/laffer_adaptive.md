...
def draw_iterations(p0s, m0s, model, line_params, π_bars, num_steps):
...
Let's simulate the result generated by setting the initial $p_{-1}$ to $p_{-1}^l$
...
π_bars = (π_l, π_u)
draw_iterations([p_l], πs, model, line_params, π_bars, num_steps=60)
```

Next we simulate the result generated by setting the initial $p_{-1}$ to $p_{-1}^u$

```{code-cell} ipython3
draw_iterations([p_u], πs, model, line_params, π_bars, num_steps=60)
```
